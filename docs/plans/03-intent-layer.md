...
| **A7** | **The pre-push `pr-self-review` gate found 0 CRITICAL but 14 WARNINGs, three of them verified functional defects. Two are fixed (see plan 07); the third, W1, is DEFERRED and recorded here.** **W2:** `clampConfidence` computed `degraded` over every source while `hasSubstance` was kind-filtered, so a `file_list` marked `truncated` past `FILE_LIST_MAX_CHARS` capped any 35–50+ file PR at `medium` forever. Fixed by giving `degraded` the same kind filter. **W3:** a REQ-7 fallback was persisted with `model: model.model` and, being cache-first, served forever — the likeliest trigger being no `OPENROUTER_API_KEY`, a supported state. Fixed by signalling the fallback out of `classifyIntent`, persisting `model: null`, and treating a null-model row older than a 15-minute window as a cache miss. | Neither fix needed a Tier A edit, a migration, or a vendor sync — **W3's fix was the design the original author already documented and left unwired**: both `db/schema/reviews.ts` and `pull.repo.ts`'s `IntentMeta` already say "a deterministic REQ-7 fallback classification may carry no model". The retry window is load-bearing: `usePrIntent` POSTs on every PR-page mount, so without it a permanently missing key turns every page view into a fresh classification plus a fresh `loadDiff`. `GET /pulls/:id/intent` deliberately does NOT adopt the miss rule — REQ-10 forbids any branch there that can spend a model call. |
| **A8** | **W1 — plan/spec documents are read from the BASE branch, not the PR head. Deferred, not fixed.** The reason is bigger than the symptom: **the clone contains no PR-head content at all.** `clone` is `--depth 1` on the default branch, `sync` does `fetch origin <defaultBranch> --depth 50` + `reset --hard`, and `readFile` is a plain `fs.readFile` over the worktree with no `ref` parameter. `fetchPullHead` — which would fetch `pull/<n>/head` — is implemented on the port and has **zero callers**: dead code, and simultaneously the nearest thing to a fix already present. So a PR that *adds* its own plan file gets ENOENT and is recorded `unreachable`; one that *modifies* a spec silently reads the stale base version — the feature under-performs precisely on the PRs it was designed for. | Recorded because the code gives no hint of any of this and it cost a full exploration to establish. **The realistic fix** is a new `GitHubClient.getFileAtRef` over `repos.getContent({ path, ref })` read at `pull.headSha`: the only option that also works for fork PRs, costing a Tier A `adapters.ts` edit + `sync-vendor.sh` + two mock updates, and **not** widening the SSRF boundary — it is the same repository already queried by `getIssue`. Reviving `fetchPullHead` instead needs three stacked changes (fetch with `--depth 1`, a `ref` parameter on `readFile` replacing `fs.readFile` with `git show`, plus ref cleanup) and still inherits shallow/fork fragility. |
| **A9** | **Found while investigating A8, not fixed and not part of any plan yet: `loadDiff`'s `git diff` path is dead in production.** `pull.headSha` is not a resolvable object in the clone (per A8) and `pull.base` is a branch *name*, not the merge-base sha, so `git diff base...head` exits `fatal: bad object`, a bare `catch {}` swallows it, and every review silently runs on a diff reconstructed from `pr_files` patches. `MockGitClient.diff()` ignores its arguments and returns a fixture, so **every test exercises the branch that never runs in production.** | This is the "a silent fail-open hides a feature that never ran" anti-pattern this very plan cites as a binding insight, one ring out. The cheap first step is to log the degradation instead of swallowing it; making the git path actually work is A8-sized or larger and also requires resolving a real merge-base. Deserves its own plan. |

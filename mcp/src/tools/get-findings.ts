...
import { projectFindingsByAgent, type AgentSelection } from '../shaping/project.js';
...
/**
 * `get_findings(repo, pr, response_format?, severity?, file?)` (ring M4) —
...
const DESCRIPTION = [
  ...
  '`repo` is "owner/name", `pr` is the pull request number. Narrow with `agent` — an id or name from ' +
    '`list_agents`, whose latest run is used — and with `severity` and `file`.',
  'Returns {agents[], counts, shown, total} — one entry per agent that reviewed this PR, each with its ' +
    'own {agent_name, reviewed, verdict, score, counts, findings[]}. A group with "reviewed": false means ' +
    'that agent has no stored result here. Findings are capped at 20 across all agents, most severe first. ' +
...
export function registerGetFindings(server: McpServer, deps: GetFindingsDeps): void {
  registerTool(
    ...
    async (args) => {
      ...
      const projected =
        format === 'detailed'
          ? projectFindingsByAgent(reviews, 'detailed', filter, selection)
          : projectFindingsByAgent(reviews, 'concise', filter, selection);

      return {
        isError: false,
        text: summarizeGroupedFindings(projected),

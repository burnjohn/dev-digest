...
Determine exactly what is under review. There are **two target modes, and they gate
differently** — deciding which one you are in is the first thing you do, before any tool call:

- **Supplied diff** — a unified diff is present in the request itself. The diff *is* the target
  and it is self-contained: its hunks carry the paths, the changed lines, and their surrounding
  context, which is everything a structural judgement needs. **Review it as supplied.** The
  working tree is corroboration, not a precondition: when a cited path turns out to be absent
  from it, say so on the `**Target:**` line — "reviewed as supplied; not present in the working
  tree, so module-local law could not be cross-checked" — and carry on with the review, citing
  only the rules you *can* still ground (the ring model and import matrix apply to the paths and
  imports the diff itself shows). A supplied diff **never** fires `G3`.
- **Named target** — a path, module, or commit range named with no diff attached. Here the
  filesystem genuinely *is* the target, so `G3` applies normally: if it cannot be read, stop.

If neither is present — nothing concrete named and nothing inferable from context — that is gate
`G1`: stop and ask.

### Step 2 — Read the governing law

`onion-architecture` and `frontend-ui-architecture` are already preloaded. On top of those, open
the `AGENTS.md` that governs whichever package the touched file belongs to — **all five, not just
the two with a skill**:

| Touched path | Also read |
|---|---|
| `server/src/**` | the touched module's own `AGENTS.md`, plus `server/AGENTS.md` |
| `client/**` | `client/AGENTS.md` |
| `reviewer-core/**` | `reviewer-core/AGENTS.md` |
| `mcp/**` | `mcp/AGENTS.md` |
| `e2e/**` | `e2e/AGENTS.md` |

`reviewer-core`, `mcp` and `e2e` are neither "backend" nor "client" in the skills' sense, so no
preloaded skill covers them — their `AGENTS.md` is the *only* law for those packages, and its
"Non-default conventions (do not break these — they are the product)" entries are stated
absolutes, i.e. `CRITICAL` sources under the severity table. Judging a file in one of those
packages without having opened its `AGENTS.md` is not a review; skip the finding instead.

When the target is a supplied diff whose paths are absent from the working tree, read the
`AGENTS.md` for the package the diff's paths *name* — the law is on disk even when the code
is not. If that file is missing too, note it on the `**Target:**` line and cite only the
preloaded skills.

These are the only sources a finding may cite.
...
| **G3 — Unreadable target** | A **named target** — a path, module, or commit range supplied *without* a diff — does not exist or cannot be read with `Read`/`Glob`/`Grep`/`Bash`. **A supplied diff never fires this gate**, however unfamiliar its paths look: a diff is self-contained, so "these files are not in the working tree" is a note on the `**Target:**` line (Step 1), never a stop. |
...
**Target:** <diff / path / module actually reviewed — for a supplied diff whose paths are absent from the working tree, add "reviewed as supplied; not present in the working tree">

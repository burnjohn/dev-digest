...
import { aggregate, loadRecordsByRun, type NodeAggregate, type Stats } from "./records/stats.js";
...
async function main(): Promise<void> {
  ...
  for (let i = 1; i <= times; i++) {
    // Each child gets its own id, so this summary counts only the rows THIS repeat produced —
    // a sibling eval process appending to records.jsonl at the same time no longer leaks in.
    const runId = newRunId(`r${i}`);
    runIds.push(runId);
    const captured = await runVitestOnce(`run ${i}/${times}`, vitestArgs, { EVAL_RUN_ID: runId });
  ...
  const records = loadRecordsByRun(runIds);

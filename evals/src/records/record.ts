...
// One id per process (per vitest run), same format as the trend reporter. The parent CLI
// (repeat/benchmark) passes EVAL_RUN_ID so it can attribute these rows back to the run it
// launched — records.jsonl is one shared append-only file, so a line offset cannot. A bare
// "vitest run" sets nothing and falls back to its own timestamp.
const RUN_ID = process.env.EVAL_RUN_ID || new Date().toISOString().replace(/[-:]/g, "").replace(/\..+/, "");
const { sha: GIT_SHA, dirty: DIRTY } = gitInfo();

const slugify = (s: string): string =>
  s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "case";

// vitest `retry` re-runs a failing test in the SAME process, so a retried case wrote its second
// attempt over the first: same run id, same slug, same path. That silently destroyed the only
// artifact worth reading — the FAILING attempt's output — leaving the passing retry behind and
// making "why did it fail?" unanswerable. Suffix every attempt after the first (`-a2`, `-a3`, …)
// so each one keeps its own file. The jsonl row already carries the per-attempt result; this just
// stops the prose from being clobbered.
...
export function record(label: string, data: RecordData): void {
  ...
  const outputFile = join("outputs", RUN_ID, `${slug}${attempt > 1 ? `-a${attempt}` : ""}.md`);

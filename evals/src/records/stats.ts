 ...
 *
 * UNSAFE WHEN ANOTHER EVAL PROCESS MAY BE RUNNING: the offset is a position in one shared
 * append-only file, so a concurrent run's rows land inside the slice. Use `loadRecordsByRun`.
 */
...
/**
 * Load only the rows written by the given run ids — the concurrency-safe way to ask "what did
 * MY run produce". A line-offset watermark cannot answer that: two eval processes appending at
 * once interleave, and the offset sweeps in the neighbour's rows (a 4-case repeat reporting
 * "6/9 cases", with a sibling suite's case names in the breakdown).
 */
export function loadRecordsByRun(runIds: Iterable<string>): EvalRecord[] {
  const want = new Set(runIds);
  if (want.size === 0) return [];
  return loadRecords().filter((r) => want.has(r.run_id));
}

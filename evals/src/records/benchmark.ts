...
import { countTests, newRunId, runVitestOnce } from "../run-vitest.js";
 ...
 *  results/outputs/. */
async function runConfig(config: string, times: number, vitestArgs: string[]): Promise<string[]> {
  const runIds: string[] = [];
  for (let i = 1; i <= times; i++) {
    const label = `${config.padEnd(9)} ${i}/${times}`;
    // Own id per child run — see loadRecordsByRun: a line offset mis-attributes a concurrent
    // eval process's rows to this benchmark.
    const runId = newRunId(`${config}-${i}`);
    runIds.push(runId);
    const captured = await runVitestOnce(label, vitestArgs, { EVAL_CONFIG: config, EVAL_RUN_ID: runId });
    const fresh = loadRecordsByRun([runId]);
    if (fresh.length === 0) {
...
async function main(): Promise<void> {
  ...
  const candIds = await runConfig("candidate", times, vitestArgs);
  const baseIds = await runConfig("baseline", times, vitestArgs);

  const configs = byConfig(loadRecordsByRun([...candIds, ...baseIds]));

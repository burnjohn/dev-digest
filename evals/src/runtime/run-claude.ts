        ...
        for (const block of msg.message.content as any[]) {
          if (block.type === "text") textParts.push(block.text);
            ...
            if (block.name === "Read") {
              ...
              // Normalize to forward slashes at capture time: every expectFilesRead/skill-file
              // check compares via a plain substring includes(), and on Windows the tool reports
              // native backslash paths — an unnormalized comparison silently fails even when the
              // file WAS read, depending on which separator the model happened to emit that turn.

...
Already wired: [`.github/workflows/evals.yml`](../.github/workflows/evals.yml). It is
**change-scoped** — `scripts/ci-detect.mjs` reads the PR diff and runs only the suites that diff
implicates, so a one-skill PR pays for one skill.
...
```bash
gh workflow run evals -f tool_model=deepseek/deepseek-chat -f content_model=deepseek/deepseek-chat
```

To move the *defaults*, edit the four model `outputs:` on the `detect` job. They live there rather
than in a workflow-level `env:` because GitHub's `env` context is not readable from a job-level
`env:` block, and `needs.detect.outputs.*` is.

**Setup.** Put the OpenRouter key in the repo's Actions secrets as `OPENROUTER_API_KEY`
(Settings → Secrets and variables → Actions). Without it every model-backed job skips cleanly and
says so in the Job Summary — which is also what a fork PR does, since forks get no secrets.

**Every model-backed job is non-blocking** (`continue-on-error` on the run step). `activation`
cases are behaviour-shaped and cheap models get rate-limited (both caveats above), so a red run is
a signal to read, not a merge gate. The honest per-suite outcome goes to the Job Summary and the
whole `results/` tree is uploaded as an artifact. `eval:quality` is static and free — it is the
natural first job to promote to a required check.

**A changed skill or agent with no evals is a SKIP, not a failure.** `ci-detect.mjs` reports it on
`skipped_skills` / `skipped_agents` and the summary prints `SKIP skill <name> — no evals written`.

**Cost control baked in.** Jobs invoke the explicit `<n>/<n>.eval.ts` path, never the bare
directory — vitest patterns are substring matches, so `agents/architecture-reviewer` would also
run `architecture-reviewer-lite` and quietly double the spend. The workflow tier adds
`--no-file-parallelism` for the rate-limit caveat above.

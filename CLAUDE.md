...
## Modules (each has its own CLAUDE.md, auto-loaded when you touch it)

| Folder           | Package                    | What it is                                   | Port | Map                                  |
|------------------|----------------------------|----------------------------------------------|------|--------------------------------------|
| `server/`        | `@devdigest/api`           | Fastify 5 + Drizzle/Postgres (pgvector)      | 3001 | [server/CLAUDE.md](server/CLAUDE.md) |
| `client/`        | `@devdigest/web`           | Next.js 15 studio (App Router, React 19)     | 3000 | [client/CLAUDE.md](client/CLAUDE.md) |
| `reviewer-core/` | `@devdigest/reviewer-core` | Pure engine: diff → prompt → LLM → findings  | —    | [reviewer-core/CLAUDE.md](reviewer-core/CLAUDE.md) |
...
- `agent-runner/dist/` **is** committed on purpose (GitHub runs it with no build step).
